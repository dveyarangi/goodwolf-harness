"""Core reaches a tree that is not its own repository, from a fresh clone of that repository at a ref.
    ...
    python .agents/scripts/harness.py <target> --install [--from REPOSITORY] [--at REF]
    python .agents/scripts/harness.py <target> --update [--overwrite] [--from REPOSITORY] [--at REF]
    python .agents/scripts/harness.py <target> --check [--from REPOSITORY]
...
def _script_gate(target: Path, script: str) -> dict:
    done = _python(target, [str(target / ".agents" / "scripts" / script), "--check"])
