...
Ran `/align` on [01-0011.0010](../tickets/done/01-0011.0010-mechanism-declared.md). It **amended the spec accepted the day before** rather than applying it. The turn that did it was the user's instruction to go read Life properly: its `/mechanism` skill, `mechanism-shape` doc and register, `evidence-sidecars`, `maintenance` with its rules file, and `/mechanism`'s own `EVIDENCE.md`. Three things this spec had reasoned to independently came back wrong.
...
- **The shape lives in `/mechanism`'s body, not in a doc.** The cut is *when is this read*, and the shape is read while incepting or amending one. This inverts what [01-0011.0010](../tickets/done/01-0011.0010-mechanism-declared.md) was minted saying.
