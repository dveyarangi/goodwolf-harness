...
on 2026-09-09**, and the next pass is the joint `/align` of
[01-0010.0130](01-0010.0130-harness-installs-into-another-tree.md) and
[01-0010.0110](01-0010.0110-project-facets-injected.md); `.0050` constrains it — its allowlist is the
question *what ships* — without blocking it. **Next cycle needs a nod**
...
| [Core stands alone, and a check says so at home](01-0010.0140-core-stands-alone.md) | Ready (own align precedes) | HITL | A reference from core into the instance half fails at home, on every change, instead of on the next port: the tree states which reaches out of `.agents/` are legitimate, a check in the verification set holds every core file to that statement, and the references that are not legitimate are gone. |
