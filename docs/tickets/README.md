...
| [A rule has one home and is installed, not copied](01-0011.0020-rules-one-home.md) | Ready (aligned 2026-09-08) | AFK | A mechanism's rules live in one file and a single generic installer writes them as visibly owned blocks into skills the mechanism does not own, removes them leaving each target byte-identical, refuses rather than guesses, and reports a hand-edited block as drift rather than accepting it as a second opinion. |
